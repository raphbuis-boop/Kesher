...
function fileIcon(url: string) {
  if (IMAGE_RE.test(url)) {
    return (
      <svg aria-hidden className="h-3.5 w-3.5 text-text-subtle" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
function ChannelIcon({ channel, className }: { channel: Channel; className?: string }) {
  if (channel === "email") {
    return (
      <svg aria-hidden className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  function AudienceRow({ a }: { a: AudienceOption }) {
    ...
    return (
      ...
      >
        ...
        >
          {checked && (
            <svg aria-hidden className="h-2.5 w-2.5 text-primary-fg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
        ...
        <div className="absolute left-0 right-0 top-full z-20 mt-1.5 max-h-80 overflow-y-auto rounded-xl border border-border bg-surface shadow-lg">
          ...
          <div className="sticky top-0 border-b border-border-subtle bg-surface px-3 py-2">
            <div className="relative">
              <svg aria-hidden className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-text-subtle" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export function ComposeFlow({
  ...
  useDialogFocus(confirmRecipients !== null, confirmPanelRef);
  useEffect(() => {
    if (confirmRecipients === null) return;
    const onKey = (e: KeyboardEvent) => { if (e.key === "Escape") setConfirmRecipients(null); };
              ...
              <div className="mt-2.5 flex flex-wrap items-center gap-2">
                {selectedAudiences.map((a) => (
                  <span key={a.slug} className="inline-flex items-center gap-1.5 rounded-full bg-surface-2 py-1 pl-3 pr-2 text-xs font-medium text-text-secondary">
                    ...
                    >
                      <svg aria-hidden className="h-2 w-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
